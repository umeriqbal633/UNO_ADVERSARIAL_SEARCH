...
class UNOGame:
    ...
    def step(self, user_card=None):
        ...
        lines.append(f"AI hand:")
        for c in state.hands[current]:
            lines.append(str(c))
        ...
        if current == 0:
            ...
            lines.append(f"\nAI decision(All possible decisions considered at depth {self.DEPTH}):")
            if valid:
                for card in valid:
            ...
            if not valid:
                lines.append(f"\nPlay: Draw")
                lines.append(f"Expected score: {round(score,2)}")
                
            lines.append(f"\n→ DECISION: {'Play ' + str(action) if action else 'Draw'} (score={round(score,2)})")
            ...
            for card, sc in decisions:
                label = str(card) if card else "Draw"
                lines.append(f"\nPlay: {label}")
                lines.append(f"Expected score: {round(sc,2)}")
            lines.append(f"\n→ DECISION: {'Play ' + str(action) if action else 'Draw'} (expected_score={round(score,2)})")
